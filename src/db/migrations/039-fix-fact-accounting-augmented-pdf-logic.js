...
async function up() {
  logger.info('Running migration: 039-fix-fact-accounting-augmented-pdf-logic');
  
  try {
    // Update the fact_accounting_augmented view to only show invoice_notes and pdf_url when PDFs are confirmed
    await db.query(`
      ...
      SELECT
        ...
        CASE 
          ...
          ELSE NULL  -- No invoice_notes if no PDF available
        ...
        CASE 
          ...
          ELSE NULL  -- No PDF URL if no PDF available
        END AS pdf_url,
        ...
        CASE 
          WHEN ae.entry_type = 'customerInvoice' THEN TRUE  -- Customer invoices always have PDFs
    ...
    logger.info('Migration 039-fix-fact-accounting-augmented-pdf-logic completed successfully');
  } catch (error) {
    logger.error('Error running migration 039-fix-fact-accounting-augmented-pdf-logic:', error.message);
    throw error;
  }
}

async function down() {
  logger.info('Reverting migration: 039-fix-fact-accounting-augmented-pdf-logic');
  ...
  try {
    ...
    logger.info('Migration 039-fix-fact-accounting-augmented-pdf-logic reverted successfully');
  } catch (error) {
    logger.error('Error reverting migration 039-fix-fact-accounting-augmented-pdf-logic:', error.message);
    throw error;
  }
}
